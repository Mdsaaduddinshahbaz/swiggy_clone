...
db.resturants_items.createIndex({ resturant_id: 1, available: 1 })
db.seller_orders.createIndex({ restaurant_id: 1, time: -1 })
db.seller_orders.createIndex({ parent_order_id: 1 })
db.Orders.createIndex({ user_id: 1, time: -1 })
db.categories.createIndex({ restaurant_id: 1 }, { unique: true })
// keep what you already have: users.email, owners.email, restaurants_name.location
...
def store_orders(userid, pickup_time=None):
    ...
    # This will contain the server-verified cart
    verified_cart = {}

    # ---------------------------------------------------------
    # 1. VERIFY CART + PREPARE INVENTORY UPDATES
    # ---------------------------------------------------------
    ...
    for restaurant_id, restaurant in items["cart"].items():
        ...
        item_ids = [
            ObjectId(iid)
            for iid in restaurant["items"].keys()
        ]

        # IMPORTANT:
        # item_qty is the actual stock quantity
        ...
        for item_id, cart_item in restaurant["items"].items():
            ...
            if not fresh.get("available", False):
                return {
                    "success": False,
                    "message": f"{fresh.get('item_name', 'Item')} is unavailable"
                }
            ...
            if fresh["item_qty"] < requested_qty:
                return {
                    "success": False,
                    "message": (
                        f"{fresh.get('item_name', 'Item')} "
                        f"has only {fresh['item_qty']} left"
                    )
                }
            ...
            if fresh["price"] != cart_item["price"]:

                print(
                    f"PRICE MISMATCH "
                    f"user={userid} "
                    f"item={item_id} "
                    f"cart={cart_item['price']} "
                ...
                return {
                    ...
                    "message": (
                        f"Price changed for "
                        f"{fresh.get('item_name', 'an item')}, "
                        f"please review your cart"
                    )
                }

            # -------------------------------------------------
            # SERVER-VERIFIED ITEM
            # -------------------------------------------------

            verified_item = dict(cart_item)

            # Never trust price from Redis/cart
            verified_item["price"] = fresh["price"]

            # Optional: use authoritative item name
            verified_item["item"] = fresh.get(
                "item_name",
                cart_item.get("item", "Item")
            )
            ...
            #
            # means two simultaneous checkouts cannot both
            # successfully take stock that doesn't exist.
            #
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            with client.start_session() as session:
                ...
                with session.start_transaction():
                    ...
                    if inventory_updates:
                        ...
                        # Every inventory update MUST match
                        # if (
                        #     inventory_result.matched_count
            ...
            print("operation failure",e,flush=True)
            error_labels = e.details.get(
                "errorLabels",
                []
            )
